...
trend_data<-subset(trend_data,seq<=641)
...
mean<-rollapply(trend_data[,keyword], 4, mean)
sd<-rollapply(trend_data[,keyword], 4, sd)
...
trend_data_EM[, EMA := fn_EMA(목도리, EMA)]
...
trend_data_EM[, EMS := fn_EMS(목도리, EMS)]

# n 은 S.D의 갯수를 의미. 해당 값이 클수록 둔감하게 이상치를 측정.
trend_data_EM<-data.frame(trend_data_EM)
n<-1
trend_data_EM$alarm <- abs(trend_data_EM[,keyword]-trend_data_EM$EMA) > n * trend_data_EM$EMS

# 전체 기간동안의 시계열 데이터와 이상치로 측정된 시점을 표기하는 plot
ggplot(subset(trend_data_EM), aes(x = week, y= 목도리)) + 
  geom_point(aes(colour=factor(alarm), size=10)) + geom_line(aes(y = 목도리), colour="black")

# 특정 기간동안의 시계열 데이터와 이상치로 측정된 시점을 표기하는 plot
 ggplot(subset(trend_data_EM,seq<=641 & seq>=541), aes(x = week, y= 목도리)) + 
  geom_point(aes(colour=factor(alarm), size=10)) + geom_line(aes(y = 목도리), colour="black") 
